...
import { IBillingValidation } from './interface';
...
export abstract class ValidationModel {
  private static _validations :{ [className: string] :{ [property :string] :IBillingValidation } } = {};
  ...
  static validate(property :string, validations :IBillingValidation) {
  ...
  addError(property :string, mapKey: string, params :any = {}) {
    ...
    this.errors[property].push({ error: mapKey, message: message });
  }
      ...
      for (let v in validations) {
        switch (v) {
          case 'presence':
            ...
            break;
          case 'greaterThan':
            let count = validations.greaterThan;
